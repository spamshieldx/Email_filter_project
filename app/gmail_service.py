...
# We need modify scope because we change labels. Keep read/modify.
...
def login():
    try:
        ...
        return jsonify({"error": "oauth_init_failed"}), 500
...
def get_gmail_service(token_info):
    if not token_info:
        current_app.logger.warning("No credentials found for Gmail service creation")
    ...
    if not creds.valid and creds.refresh_token:
        try:
            ...
            current_app.logger.exception("Failed to refresh credentials")
            raise
    return build('gmail', 'v1', credentials=creds)

def fetch_messages(service, max_results=20):
    """
    Fetch message list (IDs). Returns list of messages with id and threadId.
    """
    try:
        results = service.users().messages().list(userId='me', maxResults=max_results).execute()
        return results.get('messages', []) or []
    except Exception as e:
        current_app.logger.exception("Failed to fetch messages from Gmail")
        return []

def get_message_details(service, message_id):
    """
    Return headers (list of dicts), snippet, payload
    """
    try:
...
def modify_labels(service, msg_id, classification):
    ...
    Uses Gmail modify API safely.
    """
    try:
        if classification and classification.upper() == "SPAM":
